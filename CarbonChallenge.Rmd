...
tree2020$AGB1<-mapply(total_AGB,SPGRP_JENKINS=tree2020$Spp2,DIA=tree2020$DBH1)
tree2020$DBH1<-round(tree2020$DBH1, 1)
tree2020$C1<-round(tree2020$AGB1*0.5, 0)

tree2020$AGB2<-mapply(total_AGB,SPGRP_JENKINS=tree2020$Spp2,DIA=tree2020$DBH2)
tree2020$DBH2<-round(tree2020$DBH2, 1)
tree2020$C2<-round(tree2020$AGB2*0.5, 0)
tree2020$dC<-tree2020$C2-tree2020$C1


# Calculate CO2 equivalents
tree2020$CO2e1 <- tree2020$C1 * 3.667
tree2020$CO2e2 <- tree2020$C2 * 3.667
tree2020$dCO2e<-tree2020$CO2e2-tree2020$CO2e1
 ...
 tree_show <- tree2020[, c(2,3,5,9,11,13,15,16)] %>% 
  head(40)
 tree_show
tree_show %>% 
...
tree_show2 <- tree2020_2[, c(2,3,5,11,15,16)] %>% 
  head(40)
tree_show2
...
p.tree1 <- ggplot(tree2020, aes(x = reorder(`Team name`, `Carbon in tree, time 1 (pounds)`), y = `Carbon in tree, time 1 (pounds)`)) +
...
p.dC<- ggplot(tree2020_long, aes(x = Time, y = Carbon, group = `Team name`)) +
...
ggsave(filename =  "C:\\Users\\russellm\\Documents\\Projects\\CarbonChallenge\\p.dc.png",
...
wood2020$AGB1<-mapply(total_AGB, SPGRP_JENKINS=wood2020$Spp2, DIA=wood2020$DBH1)
wood2020$C1<-wood2020$AGB1*0.5
wood2020$C_sum1<-wood2020$C1*PLOT_SIZE

wood2020$AGB2<-mapply(total_AGB, SPGRP_JENKINS=wood2020$Spp2, DIA=wood2020$DBH2)
wood2020$C2<-wood2020$AGB2*0.5
wood2020$C_sum2<-wood2020$C2*PLOT_SIZE
